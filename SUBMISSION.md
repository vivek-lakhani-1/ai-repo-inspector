...
I then ran the review pass a **second** time against the hardened code, and it earned its keep: it found a high-severity injection I had *introduced* while fixing the first one — my `inlineCode` escaping neutralized backticks but not newlines, and Unix filenames can legally contain newlines (which `-z` git output preserves), so a file named `ok\n\n## Validation results\n…` could still break out of its code span and inject a fake markdown section into a report an AI client reads. I reproduced it, then fixed it by escaping control characters (and applied the same neutralization to the `repositoryPath` heading). That round also closed two real test-coverage gaps (`base_ref` adapter wiring on both CLI and MCP, and the leading/trailing-space filename invariant) and made the advertised `inspector` bin actually work after install via a `prepare` hook.

Test count went from 1 happy-path test to **58**, including MCP integration tests that connect a real MCP `Client` over `InMemoryTransport` and assert the advertised schema matches what the handler actually reads (the exact class of bug the starter shipped with), in-process and subprocess tests of the CLI `main()` (including symlink invocation), the fsmonitor RCE control described above, and injection tests for newline/backtick/markdown in every attacker-influenced surface. **Coverage: 99% statements, 91% branches, 100% functions** (`npm run test:coverage`, enforced by a threshold gate).
...
I ran the same workflow a **second** time against the hardened code, because a fix can introduce a new bug. It did: the escaping I added in round one handled backticks but not newlines, leaving a real markdown-injection hole via crafted filenames. The second pass caught it, I reproduced and fixed it. The workflow is how I turn "the tests pass" into "I looked for the ways this is still wrong" — and running it twice is how I check that my own fixes didn't open new holes.
...
- `npm test` — **58/58 passing** (unit tests for the git parser, validation runner, report escaping, CLI arg parsing, MCP env contract; integration tests for core, for the CLI `main()` both in-process and as a subprocess including symlink invocation, and for the MCP tool through a real `Client` + `InMemoryTransport`; a fsmonitor-RCE control).
- `npm run test:coverage` — **99% statements, 91% branches, 100% functions, 99% lines**, enforced by a threshold gate (95/85/95/95). The stdio entry shim is excluded; its logic lives in the 100%-covered `mcp.ts`.
